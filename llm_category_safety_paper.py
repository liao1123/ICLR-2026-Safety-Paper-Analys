...
async def run_one(paper_info, openai_client, semaphore, max_retries=5):
    ...
    for attempt in range(max_retries):
        try:
            async with semaphore:  # 控制并发量（一次 run_one 内顺序调用两次）
...
async def main():
    ...
    print(f"Loaded {len(safety_papers_info_data)} iclr 2026 safety related papers.")
    ...
    if os.path.exists(llm_category_save_path):
        print(f"LLM分类结果文件已存在: {llm_category_save_path}")
    else:
        # 进一步去细化文章分类，并发调用llm来进行分类
        max_concurrent = 1000
        max_retries = 20
        semaphore = asyncio.Semaphore(max(10, max_concurrent))

        tasks = [asyncio.create_task(run_one(paper_info, openai_client, semaphore, max_retries)) for paper_info in safety_papers_info_data]
